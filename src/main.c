...
static int board_init(void)
{
	...
	*/
	...
	ret = i2c_board_test();
	if (ret < 0) {
		printk("WARN: i2c board test failed (%d), continue...\n", ret);
	}
	...
	ret = oled_test();
	if (ret < 0) {
		printk("WARN: OLED test failed (%d), system continues "
		       "without display\n", ret);
	}

	/*
	 * 【重要】board_init() 必须显式 return 0。
	 *
	 * 在 ARM Cortex-M (GCC) 上，返回 int 的函数如果缺少 return 语句，
